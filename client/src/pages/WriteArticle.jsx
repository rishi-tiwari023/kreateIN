...
const WriteArticle = () => {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try{
      setLoading(true)
      const prompt = `Write a comprehensive and engaging article about ${input} in approximately ${selectedLength.text}. Format the response in Markdown with appropriate headings and sections. Return ONLY the article content, without any introductory text.`
    ...
    <div className='h-full overflow-y-scroll p-6 flex items-start flex wrap gap-4 text-slate-700'>
      ...
      border-gray-200'>
        <div className='flex items-center gap-3'>
          <Sparkles className='w-6 text-[#4A71FF]'/>
        ...
        rounded-md border border-gray-300' placeholder='The Impace of AI in Computer Science & Engg..... '
         required />
        ...
        <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
          {articleLength.map((item,index) => (
            ...
            className={`text-xs px-4 py-1 border rounded-full cursor-pointer
              ...
              'text-gray-500 border-graay-300'}`} key={index}>{item.text}</span>
          ))}
        </div>
        <br />
